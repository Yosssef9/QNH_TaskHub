      ...
      <div className="grid gap-3 sm:grid-cols-3">
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-muted-foreground text-xs font-medium">
                {t('suppliers.intelligence.suppliersShown')}
              </p>
        ...
        <Card className="p-4 sm:p-5">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-muted-foreground text-xs font-medium">
                {t('suppliers.intelligence.itemsOnPage')}
              </p>
              <p className="mt-1 text-2xl font-semibold tabular-nums">
                {(
                  data?.items.reduce((sum, item) => sum + item.purchasedItemCount, 0) ?? 0
                ).toLocaleString()}
              </p>
            </div>
            <span className="bg-primary/10 text-primary grid size-10 place-items-center rounded-xl">
        ...
        <Card className="p-4 sm:p-5">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-muted-foreground text-xs font-medium">
                {t('suppliers.intelligence.transactionsOnPage')}
              </p>
              <p className="mt-1 text-2xl font-semibold tabular-nums">
                {(
                  data?.items.reduce((sum, item) => sum + item.transactionCount, 0) ?? 0
                ).toLocaleString()}
              </p>
            </div>
            <span className="bg-primary/10 text-primary grid size-10 place-items-center rounded-xl">
      ...
      <Card className="overflow-hidden">
        <div className="flex flex-col gap-3 border-b p-4 lg:flex-row lg:items-center lg:justify-between">
          <SearchInput
            ...
            ariaLabel={t('suppliers.searchLabel')}
          />
          <Select
            value={source}
            onValueChange={(value) => {
              setSource(value as SupplierSource | 'ALL')
              setPage(1)
            }}
          >
            <SelectTrigger className="w-full lg:w-48" aria-label={t('suppliers.source')}>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              ...
              <table className="w-full min-w-[58rem] text-sm">
                <thead className="sticky top-0 z-10">
                  <tr>
                    ...
                    />
                    <SortableHeader
                      label={t('suppliers.intelligence.transactions')}
                      column="transactions"
                      sortColumn={sort.sortColumn}
                      sortDirection={sort.sortDirection}
                      onSort={onSort}
                  ...
                  {data.items.map((supplier) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-4 font-semibold tabular-nums">
                        {supplier.purchasedItemCount.toLocaleString()}
                      </td>
                      <td className="px-4 py-4 tabular-nums">
                        {supplier.transactionCount.toLocaleString()}
                      </td>
                      <td className="px-4 py-4">
            ...
            <div className="divide-y md:hidden">
              {data.items.map((supplier) => (
                <Link
                  ...
                  className="hover:bg-primary/[0.035] block p-4"
                >
                      ...
                      <div className="mt-3 grid grid-cols-3 gap-2 text-xs">
                        <div>
                          <p className="text-muted-foreground">
                            {t('suppliers.intelligence.itemsShort')}
                          </p>
                          <p className="mt-1 font-semibold">{supplier.purchasedItemCount}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">
                            {t('suppliers.intelligence.transactionsShort')}
                          </p>
                          <p className="mt-1 font-semibold">{supplier.transactionCount}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">
                            {t('suppliers.intelligence.lastPurchaseShort')}
                          </p>
