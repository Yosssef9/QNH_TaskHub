  ...
  return (
    <>
        ...
        <PopoverTrigger asChild>
          <Button variant="outline" disabled={disabled} className="h-10 w-full justify-between px-3 font-normal">
            <span className="inline-flex min-w-0 items-center gap-2">
              ...
              <span className="truncate">{selected?.name ?? selectedName ?? t('suppliers.selectPlaceholder')}</span>
            </span>
        ...
        <PopoverContent align="start" className="w-[min(30rem,calc(100vw-3rem))] p-3">
          ...
          <div className="mt-3 max-h-60 space-y-1 overflow-y-auto">
            {suppliers.data?.items.map((supplier) => (
              ...
              >
                <span className="bg-primary/10 text-primary grid size-7 shrink-0 place-items-center rounded-md"><Building2 aria-hidden="true" className="size-4" /></span>
                <span className="min-w-0 flex-1">
                  <span className="block truncate font-medium">{supplier.name}</span>
                  <span dir="ltr" className="text-muted-foreground block truncate text-xs">{supplier.code}</span>
                </span>
          ...
          <Button variant="ghost" className="mt-2 w-full justify-start" onClick={() => { setOpen(false); setCreateOpen(true) }}>
            ...
            {search.trim() ? t('suppliers.addNamed', { name: search.trim() }) : t('suppliers.add')}
          </Button>
        </PopoverContent>
      </Popover>
      ...
      <SupplierEditorDialog
        ...
        onSaved={(supplier) => { onChange(supplier); setCreateOpen(false); setSearch('') }}
      />
    </>
  )
}
