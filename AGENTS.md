...
- Phase 3 connects Organizer requests and Coordinator decisions to that engine: Organizer requests are `PENDING_APPROVAL`; Coordinators may adjust only scheduling fields before approve/reject and may schedule direct Meetings without self-approval; relationship-aware server responses keep unrelated Meeting content masked.
- Phase 4 adds the Meeting workspace lifecycle: full relationship-authorized details, revision-based Organizer reschedule requests, Organizer cancellation, protected Meeting attachments, and private personal Meeting Templates. A pending reschedule never replaces the current approved reservation until Coordinator approval succeeds atomically; rejection leaves the current reservation unchanged. Meeting files are Organizer-managed, attendees may read scheduled/cancelled Meeting files, and Coordinator file access exists only while coordinating a pending initial/reschedule request. Templates remain private to their owner and never include files in the MVP.
- Phase 5 integrates Meetings into the existing TaskHub Calendar rather than creating a second calendar source of truth. Calendar sources are composable Personal Tasks, KPI Tasks, and Meetings. Meeting-enabled Calendar views add Month, Week TimeGrid, and Day TimeGrid with 30-minute visual slots and room filtering; date-only Tasks/KPI Tasks stay in the all-day/date area. Normal users receive only attended Meetings, while Organizer/Coordinator scheduling views may receive only server-masked Busy + Organizer + room/time data for unrelated scheduled Meetings. Clicking a Month date while Meetings are enabled opens that date's Day TimeGrid. Meeting drag-to-reschedule remains disabled; lifecycle changes continue through the explicit Meetings workflow.
