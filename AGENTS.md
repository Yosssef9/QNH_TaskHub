...
- Meeting description/purpose and structured Agenda are separate. Agenda items are ordered Meeting-owned records: Topic is required; presenter/topic owner and planned duration are optional. A presenter must be the Organizer or a selected attendee. Planned agenda time is UX guidance only and may exceed Meeting duration without blocking save or scheduling. Keep Agenda separate from later Notes/Decisions and Action Items.
...
- The notification bell uses periodic TanStack Query refresh; do not add WebSockets unless a later collaboration requirement justifies them. The 15-minute Meeting reminder is in-app only, defaults on per user, and is generated only for the Organizer/attendees of a still-scheduled current revision. Reschedule/cancellation or disabling the personal preference invalidates stale reminders.
