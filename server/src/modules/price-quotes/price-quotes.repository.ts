  ...
  return `
    ...
    OUTER APPLY (
      ...
      WHERE TRY_CONVERT(BIGINT, tx.ITEM_NO) = quote_row.item_id
        ...
        AND ISNULL(NULLIF(LTRIM(RTRIM(CONVERT(NVARCHAR(30), tx.CURRENCY_CODE))), N''), N'') COLLATE DATABASE_DEFAULT = quote_row.currency_code COLLATE DATABASE_DEFAULT
        AND ISNULL(NULLIF(LTRIM(RTRIM(CONVERT(NVARCHAR(100), tx.UNIT_NAME_EN))), N''), N'') COLLATE DATABASE_DEFAULT = quote_row.unit_name COLLATE DATABASE_DEFAULT
      ORDER BY
  ...
  const summaries = await s.query<QuoteSupplierSummaryRecord>(`
    ...
    OUTER APPLY (
      ...
      WHERE (@itemId IS NULL OR TRY_CONVERT(BIGINT, tx.ITEM_NO) = @itemId)
        ...
        AND TRY_CONVERT(DECIMAL(19,6), tx.UNIT_COST) IS NOT NULL
        AND TRY_CONVERT(DATE, tx.DELIVERY_NOTE_DATE) IS NOT NULL
        AND ISNULL(NULLIF(LTRIM(RTRIM(CONVERT(NVARCHAR(30), tx.CURRENCY_CODE))), N''), N'') COLLATE DATABASE_DEFAULT = grouped.currency_code COLLATE DATABASE_DEFAULT
        AND ISNULL(NULLIF(LTRIM(RTRIM(CONVERT(NVARCHAR(100), tx.UNIT_NAME_EN))), N''), N'') COLLATE DATABASE_DEFAULT = grouped.unit_name COLLATE DATABASE_DEFAULT
    ...
    compared AS (
      ...
      OUTER APPLY (
        ...
        WHERE TRY_CONVERT(BIGINT, tx.ITEM_NO) = scoped.item_id
          ...
          AND TRY_CONVERT(DECIMAL(19,6), tx.UNIT_COST) IS NOT NULL
          AND TRY_CONVERT(DATE, tx.DELIVERY_NOTE_DATE) IS NOT NULL
          AND ISNULL(NULLIF(LTRIM(RTRIM(CONVERT(NVARCHAR(30), tx.CURRENCY_CODE))), N''), N'') COLLATE DATABASE_DEFAULT = scoped.currency_code COLLATE DATABASE_DEFAULT
          AND ISNULL(NULLIF(LTRIM(RTRIM(CONVERT(NVARCHAR(100), tx.UNIT_NAME_EN))), N''), N'') COLLATE DATABASE_DEFAULT = scoped.unit_name COLLATE DATABASE_DEFAULT
        ORDER BY TRY_CONVERT(DATE, tx.DELIVERY_NOTE_DATE) DESC,
                 TRY_CONVERT(DATETIME2(3), tx.CONFIRM_DATE) DESC,
                 CONVERT(NVARCHAR(120), tx.INVOICE_NO) DESC,
                 CONVERT(NVARCHAR(120), tx.ORDER_ID) DESC
