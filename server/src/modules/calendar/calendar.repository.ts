...
export const calendarRepository = {
  ...
  async searchTasks(
    ...
    limit: number,
  ): Promise<CalendarSearchTaskRecord[]> {
    const pool = await getDatabasePool();
    const result = await pool
      .request()
      .input("ownerUserId", sql.Int, ownerUserId)
      ...
      .query<CalendarSearchTaskRecord>(`
        SELECT TOP (@limit)
          task.id,
          task.title,
          task.status,
        ...
        WHERE task.owner_user_id = @ownerUserId
          ...
          AND COALESCE(task.due_date, task.start_date) IS NOT NULL
          AND (
            task.title LIKE @search ESCAPE '\\'
            OR task.description LIKE @search ESCAPE '\\'
            OR (@scope = 'PERSONAL' AND list.name LIKE @search ESCAPE '\\')
            OR (@scope = 'KPI' AND instance.name_snapshot LIKE @search ESCAPE '\\')
            OR (@scope = 'KPI' AND cycle.title LIKE @search ESCAPE '\\')
          )
          AND (@status IS NULL OR task.status = @status)
          AND (@priority IS NULL OR task.priority = @priority)
          AND (
        ...
        ORDER BY
          CASE WHEN COALESCE(task.due_date, task.start_date) >= @today THEN 0 ELSE 1 END,
          CASE WHEN COALESCE(task.due_date, task.start_date) >= @today
            THEN COALESCE(task.due_date, task.start_date) END ASC,
          CASE WHEN COALESCE(task.due_date, task.start_date) < @today
            THEN COALESCE(task.due_date, task.start_date) END DESC,
          CASE task.priority WHEN 'HIGH' THEN 1 WHEN 'MEDIUM' THEN 2 ELSE 3 END,
          task.title,
          task.id;
      `);
