...
const select = `task.id,task.list_id AS listId,task.kpi_instance_id AS kpiInstanceId,
  ...
  NULL AS actionMeetingId,
  NULL AS actionMeetingTitle,
  NULL AS actionAssigneeUserId,
  NULL AS actionAssigneeName,
  NULL AS actionAssignedByUserId,
  NULL AS actionAssignedByName,
  NULL AS actionAgendaItemId,
  NULL AS actionAgendaTitle,
  NULL AS actionAssignedAtUtc`;
...
export const kpiWorkRepository = {
  async listAll(owner: number, query: GlobalKpiTaskListQuery, today: string) {
    ...
    const request = bind(pool.request(), owner, query, today)
      .input("cycle", sql.BigInt, query.cycleId ?? null)
      .input("kpi", sql.BigInt, query.kpiId ?? null)
      .input("offset", sql.Int, (query.page - 1) * query.pageSize)
      .input("size", sql.Int, query.pageSize);
    const records =
      await request.query<TaskRecord>(`SELECT ${select} FROM dbo.TM_tasks task ${joins} ${apply}
    ...
    const count = await bind(pool.request(), owner, query, today)
      ...
      .input("kpi", sql.BigInt, query.kpiId ?? null)
      .query<{ total: number }>(
        `SELECT COUNT_BIG(*) total FROM dbo.TM_tasks task ${joins} WHERE ${where};`,
      );
  ...
  async list(owner: number, instanceId: number, query: TaskListQuery, today: string) {
    ...
    const request = bind(pool.request(), owner, query, today)
      .input("instance", sql.BigInt, instanceId)
      .input("offset", sql.Int, (query.page - 1) * query.pageSize)
      .input("size", sql.Int, query.pageSize);
    const records =
      await request.query<TaskRecord>(`SELECT ${select} FROM dbo.TM_tasks task ${joins} ${apply}
  ...
  async create(tx: DatabaseTransaction, owner: number, instanceId: number, input: PersistedInput) {
    const result = await tx
      ...
      .input("due", sql.Date, input.dueDate)
      .input("reference", sql.Date, input.referenceDate).query<{
      id: number;
    }>(`INSERT dbo.TM_tasks(owner_user_id,kpi_instance_id,title,description,priority,start_date,due_date,reference_date,display_order)
  ...
  async calculationTasks(
    ...
    return result.recordset.map((row) => ({
      ...
      subtaskCompleted: Number(row.subtaskCompleted),
    }));
  },

  async calculationSubtasks(
    owner: number,
    instanceId: number,
    start: string,
    end: string,
  ): Promise<CalculationSubtask[]> {
    const pool = await getDatabasePool();
    const result = await pool
      .request()
      .input("owner", sql.Int, owner)
      .input("instance", sql.BigInt, instanceId)
      .input("start", sql.Date, start)
      .input("end", sql.Date, end).query<{ dueDate: string; completedDate: string | null }>(`
  ...
  async saveManualResult(
    ...
    const merge = await tx
      ...
      .input("owner", sql.Int, owner)
      .input("instance", sql.BigInt, instanceId)
      .input("start", sql.Date, input.periodStart)
      .input("end", sql.Date, input.periodEnd)
      .input("num", sql.Decimal(19, 4), input.numeratorValue)
      .input("den", sql.Decimal(19, 4), input.denominatorValue)
      .input("value", sql.Decimal(19, 4), input.manualValue)
      .input("actual", sql.Decimal(19, 4), result.actualValue)
      .input("target", sql.Decimal(19, 4), targetValue)
      .input("direction", sql.VarChar(20), targetDirection)
