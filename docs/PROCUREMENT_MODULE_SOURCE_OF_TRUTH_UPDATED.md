...
- Supplier identity is `SUPPLIER_CODE` only. When the detected Supplier label/header has a value directly above it, treat the cell above as the authoritative Supplier Code; otherwise treat the detected Supplier header itself as the Supplier Code. Match only by exact normalized `SUPPLIER_CODE`. Supplier display names may differ from master names and must never participate in identity or fallback matching. Multiple rows for the same exact Supplier Code are `AMBIGUOUS`; no code match is `NOT_FOUND`. Never use partial/contains matching, never apply the Item leading-zero fallback to Suppliers, and never create missing master Items/Suppliers from an import.
...
- If both an Actual Purchase value and a compatible private Quote value exist for that metric, use the **lower** of the two values.
- If the Supplier has no Actual purchase history, a Quote may establish its own comparison scope.
- If only one usable source exists for the selected metric, use that available source.
- If Actual history exists but the Quote Currency/UOM differs from the Actual scope, do not use that Quote for Best Choice; use the compatible Actual metric when available.
