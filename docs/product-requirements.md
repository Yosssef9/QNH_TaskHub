...
> Status: planning baseline. Last aligned: 2026-09-12.
...
- `TM_access_permissions` is the active source of feature permissions for Procurement and the combined KPI + Work Cycles area.
...
### Default module access and KPI + Work Cycles permission

- Normal **Tasks / My Tasks** remain available to every active TaskHub user by default.
- The basic **Meetings** workspace also remains available by default so users can see Meetings they are independently authorized to see; Organizer and Coordinator capabilities remain separate Meeting permissions.
- **KPIs and Work Cycles share one combined feature permission** because they form one operational workflow. The permission is stored as `KPI_MANAGEMENT / KPI_WORK_CYCLES / ACCESS` in `TM_access_permissions`.
- Without that permission, KPI Library, Work Cycles, KPI Tasks, KPI Calendar/search surfaces, dashboard KPI content, and KPI/Work Cycle notifications are not exposed. Dedicated KPI/Work Cycle APIs reject access server-side.
- `ADMIN` does not implicitly grant KPI + Work Cycles business access; administrators assign the combined permission explicitly in Access Management.
- Migration `039_add_kpi_work_cycles_access_permission.sql` expands the existing permission-table constraints for this permission and intentionally performs no user backfill, so KPI + Work Cycles access is opt-in.
...
Meeting Follow-up is delivered in separate implementation phases without introducing a second Task or Meeting engine:

- Follow-up Phase 1 reuses normal `TM_tasks` records for Meeting Action Items and stores only the Meeting/Agenda/assignee relationship in `TM_meeting_action_items`. The Meeting Organizer remains Task owner/administrator while exactly one eligible Meeting attendee is the executor. Organizer and assignee capabilities are enforced server-side; ordinary attendees and Coordinators do not inherit private Action Item access merely from Meeting visibility. Assignees receive the normal `Assigned to me` projection and capability-aware Task Details. Task attachments record the real uploader and Task activity records the real authorized actor. In-app Action Item assignment/completion notifications target only the relevant assignee/Organizer.
- Follow-up Phase 2 adds Meeting-owned Decisions and one Organizer-controlled Meeting Notes document, plus a derived permission-aware Follow-up Summary and Agenda quick actions. The Organizer may create/edit Decisions and Notes only once the scheduled Meeting reaches its approved start time; authorized Meeting viewers may read them according to normal Meeting visibility but cannot edit another Organizer's content. Decisions may optionally reference an Agenda topic. Summary Action Item totals are calculated only from Action Items the current actor may actually access: Organizer sees all Action Items from their Meeting, an assignee sees only items assigned to them, and other attendees/Coordinators receive zero Action Item counts unless independently authorized. Hidden-item counts are never exposed.
- Follow-up Phase 3 adds the immutable creation-time `TM_meetings.follow_up_of_meeting_id` self-reference, resolves the full authorized Related Meetings family from any family member, and adds **Schedule Follow-up Meeting** by reusing the normal REQUEST/DIRECT Meeting creation workflows. The relationship grants zero authorization: each family member is independently filtered by the existing Meeting content policy with no hidden-item counts or existence leakage. Follow-up creation may prefill reusable title/description/attendee/room/duration values but starts with date/time unselected and does not copy Agenda, Decisions, Notes, Action Items, files, or activity.
- Follow-up Phase 4 replaces the earlier accordion-heavy presentation with a dashboard-style Follow-up workspace inside the existing Meeting Details tab. The overview shows permission-aware summary metrics, quick actions, compact Action Item/Decision/Meeting Notes previews, and Related Meetings. `Action Items`, `Decisions`, `Meeting Notes`, and `Related Meetings` have focused drill-down states on the same Meeting Details route using `?tab=follow-up&section=...`, so browser Back, refresh, and deep links preserve context without creating a second Meetings module. Schedule Follow-up Meeting appears once in the top quick actions rather than as a duplicate Next Meeting card, allowing Related Meetings to use the full row without an empty companion column; empty preview states remain compact instead of reserving large blank areas. The schedule-follow-up action opens the normal centered Meeting editor and keeps the Phase 3 prefill/date-time rules: title, description, attendees, room, and duration are prefilled as editable values, while date/time start unselected. No authorization, ownership, visibility, or database behavior changes in Phase 4.


- Meeting Series adds a Coordinator-only multiple-Meeting scheduling workflow. A Series is creation/grouping/audit metadata only: each occurrence is materialized as a normal independent Meeting using the existing scheduling engine, permissions, room locking, revisions, attendees, Agenda, and lifecycle. Series membership is separate from Follow-up relationships and grants no Meeting authorization. The creation composer supports bounded repeat/custom schedules and per-occurrence overrides while preserving original occurrence identity. The Coordinator-owned Series workspace lists **My Series**, opens read-only Series history/defaults plus member Meetings, links back from eligible Meeting Details, and can duplicate the original defaults/pattern into a new future scheduling draft; it does not bulk-rewrite already-created Meetings. Common Series files are stored once in protected storage and associated with each applicable Meeting rather than duplicating the binary, while occurrence-specific files remain scoped to that Meeting; all reads continue to use normal Meeting authorization and final association cleanup removes the binary only after its last active reference is gone. Initial Series communication is grouped into one recipient-specific notification/email containing only that recipient’s applicable Meetings, while later reschedule/cancellation/reminder events continue through the ordinary per-Meeting lifecycle.
...
- Drag-to-reschedule remains disabled. Phase 5 adds timed Meeting display only; Meeting scheduling/rescheduling continues through the explicit Meetings workflow. Premium resource-column scheduling, external calendar synchronization, and live master-series mutation (for example “this and following”) remain deferred.
...
- Meeting Follow-up Phase 1 Action Item relationships, real Task attachment uploader identity, authorized non-owner Task activity actors, and Action Item in-app notification types are introduced by migration 035; applying it is a separate manual database step after the prior Task/Meeting/access foundations.
- Meeting scheduling slot-interval preference is introduced by migration 036.
- Meeting Follow-up Phase 2 Decisions, Meeting Notes, and Agenda-link deletion behavior are introduced by migration 037; applying it is a separate manual database step after 036. Migration 037 does not add Related Meetings or Schedule Follow-up Meeting schema.
- Meeting Follow-up Phase 3 Meeting-to-Meeting relationship schema is introduced by migration 038; applying it is a separate manual database step after 037. The new parent relationship is set only during follow-up Meeting creation and is immutable through ordinary UI/API flows.
