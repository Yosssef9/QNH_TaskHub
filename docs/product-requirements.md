...
- Calendar Task/KPI rendering reuses the existing status, priority, overdue, light/dark, and RTL/LTR visual system. Meeting rendering uses the existing Meeting schedule response: `FULL` entries show the Meeting title and open Meeting Details; `BUSY` entries show only Busy + Organizer + room/time and are not clickable for details.
